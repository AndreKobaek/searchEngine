...
Togo probably doesn't appear very often in the corpus, and therefore if a site has just a few occurrences of "Togo" the search word counts for more than many occurrences of the word "government".
...
This solution has the \code{Score} interface as required, and another big improvement is that the score/rank field from \code{Website} is removed, and instead we use the value from \code{rank} method directly to do the sorting.  
